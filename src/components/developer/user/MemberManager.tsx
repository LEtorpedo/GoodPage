  ...
  const fetchMembers = useCallback(async () => {
    ...
    try {
      ...
      if (result.success && Array.isArray(result.data)) {
        ...
        // 按状态优先级排序并合并
        const statusOrder = ["PROFESSOR", "POSTDOC", "PHD_STUDENT", "MASTER_STUDENT", "UNDERGRADUATE", "VISITING_SCHOLAR", "RESEARCH_STAFF", "ALUMNI", "OTHER"];
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      ...
      if (draggedMember.status !== targetMember.status) {
        toast.error("Members can only be reordered within the same status group");
        return;
      }
      ...
      try {
        ...
        const sameStatusMembers = newMembers.filter(m => m.status === draggedMember.status);
        ...
        if (!result.success) {
          throw new Error(result.error?.message || "Failed to update member order");
        }

        const statusDisplayName = draggedMember.status?.toLowerCase().replace(/_/g, " ") || "member";
        toast.success(`${statusDisplayName.charAt(0).toUpperCase() + statusDisplayName.slice(1)} order updated successfully`);
      } catch (error) {
            ...
            {members.length === 0 ? (
              ...
              <>
                ...
                <div className="bg-blue-900/20 border border-blue-600/30 rounded-lg p-3 text-blue-200 text-sm">
                  <p>
                    <strong>Drag & Drop:</strong> You can reorder members within the same status group. 
                    All member order changes will be saved and reflected on the public site.
                  </p>
                </div>
                      ...
                      members.forEach((member, index) => {
                        ...
                        if (member.status !== currentStatus) {
                          ...
                          renderedElements.push(
                            <div key={`header-${currentStatus}`} className="pt-4 pb-2">
                              <div className="flex items-center space-x-2">
                                <h3 className="text-lg font-semibold text-green-400">
                                  {statusTitles[currentStatus] || currentStatus}
                                </h3>
                              </div>
                              {index > 0 && <div className="border-t border-gray-600 mt-2" />}
                            </div>
                          );
                        }
